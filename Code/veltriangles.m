...
function [a,g,d] = veltriangles(d,g,k,g2,dev,p)
...
for bb=1:2
   ...
   % Set maximum diffusion factor
   df_lim = g2.DF; df_max = 0.6; df_min = 0; df_tol = 0.001;
   while (df_max - df_min) > df_tol
...
%    scDF(1:Nr,bb) = (g2.DF - (1 - vdn./vup)) * 2 .* vup  ./ abs((vupth - vdnth));
    df_mid = 0.5 * (df_max + df_min);
   scDF(1:Nr,bb) = (df_mid - (1 - vdn./vup)) * 2 .* vup  ./ abs((vupth - vdnth));
%    g.DF = g2.DF;
   ...
   pitchmid(bb) = scDF(imid,bb) * chordmid;
...
%    g2.DF = g2.DF - (max(DF(1:Nr,bb)) - df_max);
    if max(DF(1:Nr,bb)) > df_lim
        df_max = df_mid; 
    else
        df_min = df_mid;   
    end
...
if p==1
   ...
   set(gcf,'Position',[20 100 1200 700]);
   set(gcf, 'color', 'w'); 
