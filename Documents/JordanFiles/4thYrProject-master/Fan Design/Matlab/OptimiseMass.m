function [r_cCRIT, r_h, phi, psi, omega, OPThrust, sig] = OptimiseMass(payload)
...
t           = .5e-3;         % CASING THICKNESS
r_c         = 60e-3;      % NOMINAL CASING RADIUS

turboL = 70e-3;
inletL = 20e-3;

% Motor Speed
rpm         = 8500;         % NOMINAL MOTOR SPEED

% Motor parameters
kv          = 750;         % KV CONSTANT
maxcurrent  = 15;           % Max current for motor
supplyvoltage = 15;         % Battery voltage
powerrated  = supplyvoltage*maxcurrent;          % Rated power of motors at 5cell lipo voltage

% Drone body mass
emptyweight = 0.529 + 0.467; % Chassis MASS
totalmass   = emptyweight;          % DRONE EMPTY WEIGHT
motormass   = 0.087;

% Variables to plot
variables = 'phipsi_omega';
mesh = 1;
chics = 1;
diffplot = 1;
plott = 1;
...
% Test points
n           = 400;                          % Number of test r_c data points
...
hubmass = rhoPLA*pi*turboL*(r_h^2 - (r_h - 3e-3)^2);
...
thrust = rhoAIR*pi*phi^2*omega^2*(r_c^4 - r_h^4)/(2*sig);
...
W.body = (emptyweight/W.numberofprop)*9.81 + W.motor;
W.fan = W.diff + W.case + W.hub + W.inlet;
W.payload = payload*9.81/W.numberofprop;
% W.total = (mass(critidx) + emptyweight/numberofprop)*9.81;
W.total = W.fan + W.body;
...
save('meshvars', 'geom');
save('vars', 'phi', 'psi', 'r_h', 'r_cCRIT', 't', 'rpm', 'W', 'diffgeom', 'inletgeom', 'kv', 'powerrated', 'maxcurrent');
