function [out, xy_int] = lineSegmentIntersect(XY1,XY2)
...
%       normalized distance o_1from the start point of line segment XY1(j,:) to the
...
x_int = INT_X .* INT_B; x_int = x_int(x_int ~= 0 & u_a < 1 & u_a > 0);
y_int = INT_Y .* INT_B; y_int = y_int(y_int ~= 0 & u_a < 1 & u_a > 0);
xy_int = [x_int y_int];
